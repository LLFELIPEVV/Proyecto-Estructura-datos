...
class Insights
{
	...
	public function optin()
	{
		...
		# sync fluent crm
		$this->sync_fluent_contact();
	}

	/**
	 * Send to FluentCRM webhooks
	 */
	public function sync_fluent_contact()
	{

		if (!class_exists('\WPPOOL\Product')) return false;

		$data = $this->get_tracking_data();

		$product = new \WPPOOL\Product('wp_dark_mode');
		...
		# Sync to live CRM
		$sync = $product->subscribe_free($fluent_data);		 
	}
